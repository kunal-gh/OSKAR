...
async def analyze_audio(
    ...
    try:
        def _run_pipeline(req):
            ...
            verify_res  = er.verify_claim(req.text) if claim_res["is_verifiable"] \
                          else {"verdict": "uncertain", "confidence": 0.0, "evidence": None, "graph_triples": []}
            misinfo     = float(verify_res["confidence"]) if verify_res["verdict"] == "refuted" \
                          else max(0.0, 1.0 - verify_res["confidence"]) if verify_res["verdict"] == "supported" \
                          else 0.5
            bot_score   = gnn_detector.predict(req.social_context)
...
async def analyze_image(
    ...
    try:
        def _run_pipeline(req):
            ...
            fusion      = risk_engine.calculate_risk(
                hate_score=hate_res["score"] if hate_res["label"] == "hate" else 1.0 - hate_res["score"],
                misinfo_score=misinfo, bot_score=bot_score, trust_score=trust_score
            )
            return {
                "risk_score": fusion["mean_risk"],
                "confidence_interval": fusion["confidence_interval"],
        ...
        result = ocr_analyzer.analyze(tmp_path, user_id=user_id, psm=psm, lang=lang, analyze_fn=_run_pipeline)
...
def analyze_multilingual(req: AnalyzeRequest, api_key: str = fastapi.Security(get_api_key)):
    ...
    def _run_pipeline(inner_req):
        ...
        verify_res  = er.verify_claim(inner_req.text) if claim_res["is_verifiable"] \
                      else {"verdict": "uncertain", "confidence": 0.0, "evidence": None, "graph_triples": []}
        misinfo     = float(verify_res["confidence"]) if verify_res["verdict"] == "refuted" \
                      else max(0.0, 1.0 - verify_res["confidence"]) if verify_res["verdict"] == "supported" \
                      else 0.5
        ...
        fusion      = risk_engine.calculate_risk(
            hate_score=hate_res["score"] if hate_res["label"] == "hate" else 1.0 - hate_res["score"],
            misinfo_score=misinfo, bot_score=bot_score,
            trust_score=trust_score, burst_score=burst_res["anomaly_score"]
        )
        return {
            "risk_score": fusion["mean_risk"],
            "confidence_interval": fusion["confidence_interval"],
            "route": fusion["route"],
            "components": {
                "hate": hate_res, "claim": claim_res, "verification": verify_res,
                "bot_swarm":    {"probability": bot_score, "enabled": gnn_detector.enabled},
                "burst_detect": {"anomaly_score": burst_res["anomaly_score"], "is_burst": burst_res["is_burst"]}
            },
            "trust_score": trust_score
        }
...
def log_warning_impression(req: ImpressionRequest, api_key: str = fastapi.Security(get_api_key)):
    ...
    event = warning_tracker.log_impression(
        req.user_id, risk_score=req.risk_score,
        route=req.route, content_hash=req.content_hash
    )
    return event
