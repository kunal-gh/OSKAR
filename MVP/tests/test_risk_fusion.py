...
def test_risk_fusion_schema():
    ...
    result = engine.calculate_risk(hate_score=0.9, misinfo_score=0.8, bot_score=0.0, trust_score=0.1)
...
def test_risk_fusion_logic():
    ...
    clean_res = engine.calculate_risk(hate_score=0.0, misinfo_score=0.0, bot_score=0.0, trust_score=0.9)
    assert clean_res["route"] == "auto_action"
    assert clean_res["mean_risk"] < 0.2
    
    # Untrusted user with terrible content
    toxic_res = engine.calculate_risk(hate_score=1.0, misinfo_score=0.9, bot_score=0.9, trust_score=0.1)
