...
async function ping() {
  ...
  try {
    ...
    dot.className = "sdot on"; // Keep green so it looks good for portfolio
    dot.style.background = "#c8a96e"; // Make it warning yellow-ish
    dot.style.boxShadow = "0 0 10px rgba(200,169,110,0.6)";
    label.textContent = "Demo Mode";
    if (fstat) fstat.textContent = "Backend Offline — Simulated AI";
  }
}
...
function simulateAnalysis(text) {
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      let risk = 0.12;
      let label = "opinion";
      let route = "auto_action";

      if (lower.includes("hate") || lower.includes("disgusting") || lower.includes("scum") || lower.includes("sick") || lower.includes("eradicate")) {
        risk = 0.88;
        label = "hate";
        route = "human_review";
      } else if (lower.includes("hoax") || lower.includes("fake") || lower.includes("autism") || lower.includes("flat") || lower.includes("secret")) {
        risk = 0.65;
        label = "misinfo";
        route = "soft_warning";
      }
      ...
      resolve({
        risk_score: risk,
        confidence_interval: [Math.max(0, risk - 0.1), Math.min(1.0, risk + 0.1)],
        route: route,
        trust_score: 0.85,
        components: {
          hate: { label: label, score: risk, uncertainty: 0.2 },
          claim: { is_verifiable: true, claim_type: "Assertion", confidence: 0.85 },
          verification: { verdict: "uncertain", confidence: 0.5, evidence: "Simulated Vercel Demo Mode Analysis." }
        }
      });
    }, 4500); // Simulate 4.5s async Celery latency
  });
}
...
async function runAnalysis() {
  ...
  try {
    ...
    try {
      ...
      console.warn("Backend unavailable, using Demo Mode fallback:", apiError);
